...
/* Sometimes, Unplug the headset, the headset state will still exist on the status bar. 
 * Root cuase: when unplug the headset slowly, mic pin & gnd pin will be shorted, 
 * and check_cable_type() will go to "case MIC_BIAS" and "else if(current_status == 3)", 
 * however, JRD_HDST_INSERT() still think the headset isn't be unplugged. So  
 * jrd_hdst_insert is true("jrd_hdst_insert???: 1" will be printed). */
#define JRD_FIX_PR411928
#ifdef JRD_FIX_PR411928
static int jrd_hdst_unplug_cnt = 0;
#endif

/* Almost the same with PR411928. When use a 3-node headphone, sometimes the 
 * headphone icon will not disappear. */
#define JRD_FIX_PR417773
#endif
...
#ifdef JRD_FIX_PR411928
struct timer_list jrd_hdst_unplug_check_timer;
static void jrd_hdst_unplug_check(unsigned long data)
{
    int hdst_insert = JRD_HDST_INSERT();
    ACCDET_DEBUG("[Accdet][%s]hdst_insert[%d]: %d", __func__, jrd_hdst_unplug_cnt, hdst_insert);
    if(hdst_insert) { 
        if(jrd_hdst_unplug_cnt++ < 5) {
            mod_timer(&jrd_hdst_unplug_check_timer, jiffies + 20);
        }
    } else {
        jrd_hdst_unplug_cnt = 0;
        //Disable ACCDET
    #ifdef ACCDET_MULTI_KEY_FEATURE
        //set_mic_low_power(0);
        OUTREG32(ACCDET_PWM_IDLE, 0x00);
...
#endif
#if defined(JRD_IPHONE_HDST_COMPATIBILITY) && defined(JRD_FIX_PR411928)
        init_timer(&jrd_hdst_unplug_check_timer);
        jrd_hdst_unplug_check_timer.function = &jrd_hdst_unplug_check;
        jrd_hdst_unplug_check_timer.data = ((unsigned long) 0 );
#endif
    //------------------------------------------------------------------
	//							wake lock
	//------------------------------------------------------------------
...
#if defined(JRD_IPHONE_HDST_COMPATIBILITY) && defined(JRD_FIX_PR411928)
        del_timer(&jrd_hdst_unplug_check_timer);
#endif
