...
#if DIABLOHD_COMPASS_ISSUE
//john add use for diablo_diabloHD share git issue.
	printk("john_add use for test diablo hd compass issue. tpd start before sensor\n");
...
#if DIABLO_COMPASS_ISSUE 
	//john add use for diablo_diabloHD share git issue.
	printk("john_add use for test diablo hd compass issue. tpd start after sensor\n");
#if defined(CONFIG_MTK_TOUCHPANEL)
    retval = platform_device_register(&mtk_tpd_dev);
    if (retval != 0) {
        return retval;
